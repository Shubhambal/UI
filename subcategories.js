/*
  ...
  ```
*/
// Written by [Nikhil,hitendra,shruti, omkar]
// Date: [25/05/2023]
...
export default function Subcategory() {
    const { code } = useParams(); // Retrieves the "code" parameter from the URL
    const [Category, setCategory] = useState([]); // Initializes state variable for storing categories
    const navigate = useNavigate(); // Provides the navigate function for programmatic navigation
    console.log(code + "------------------------------------------");
  
    useEffect(() => {
      // Fetches subcategories based on the code parameter
      fetch("http://localhost:8080/api/subcategories/" + code)
        .then((res) => res.json())
        .then((result) => {
          setCategory(result); // Sets the fetched subcategories to the state variable
          console.log(result);
        });
      console.log(Category); // Outputs the initial value of the Category state variable (empty array)
    }, []);
  
    const handlesubcategory = (category_Id) => {
      // Handles subcategory click event and navigates to "/productsbycat/:category_Id"
            ...
            <div className="mx-auto max-w-2xl py-10 sm:py-20 lg:max-w-none lg:py-8">
              <div className="flex flex-wrap justify-start -mx-2 items-start ">
                {Category.map((cat) => (
                  ...
                  <a key={cat.id} href={`/productsbycat/${cat.category_Id}`}>
